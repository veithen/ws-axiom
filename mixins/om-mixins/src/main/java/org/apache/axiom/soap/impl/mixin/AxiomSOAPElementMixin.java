...
public abstract class AxiomSOAPElementMixin implements AxiomSOAPElement {
    ...
    public final void checkChild(OMNode child) {
        if (child instanceof OMElement omElement && !isChildElementAllowed(omElement)) {
            throw new SOAPProcessingException(
                    child.getClass().getName()
                            + " is not allowed as a child of "
                            + getClass().getName());
        }
        if (child instanceof AxiomSOAPElement axiomSOAPElement) {
            SOAPHelper soapHelper = getSOAPHelper();
            SOAPHelper childSOAPHelper = axiomSOAPElement.getSOAPHelper();
